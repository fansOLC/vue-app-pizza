  ...
  <header>
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
      ...
      <ul class="navbar-nav ml-auto">
        <li><router-link class="nav-link" v-show="!isLogin" to="/login">登录</router-link></li>

        <li class="nav-link">{{currentUser}}</li>
        <li><router-link class="nav-link" v-show="isLogin" to="/login">[退出]</router-link></li>

        <li><router-link class="nav-link" v-show="!isLogin" to="/register">注册</router-link></li>
      </ul>
    </nav>
  </header>
</template>
